-- Final hardening for the five intentional SECURITY DEFINER business RPCs.
...
DECLARE v_id uuid;
BEGIN
  IF auth.uid() IS NULL OR NOT (private.has_approved_role(auth.uid(),'teacher') OR private.has_approved_role(auth.uid(),'admin')) THEN RAISE EXCEPTION 'Staff authorization required'; END IF;
  ...
  IF EXISTS (SELECT 1 FROM jsonb_array_elements(p_options) x WHERE jsonb_typeof(x)<>'string') THEN RAISE EXCEPTION 'Options must be strings'; END IF;
  IF (SELECT count(*) FROM jsonb_array_elements_text(p_options))<>(SELECT count(DISTINCT value) FROM jsonb_array_elements_text(p_options)) THEN RAISE EXCEPTION 'Options must be unique'; END IF;
  ...
  IF p_order_num IS NULL OR p_order_num<0 THEN RAISE EXCEPTION 'Order number must be non-negative'; END IF;
  INSERT INTO public.exercises(lesson_id,question,options,points,order_num) VALUES(p_lesson_id,trim(p_question),p_options,p_points,p_order_num) RETURNING id INTO v_id;
...
BEGIN
  ...
  IF v_code !~ '^[A-Z0-9]{6}$' THEN RAISE EXCEPTION 'código inválido'; END IF;
...
AS $$
DECLARE v_code text; v_class public.classes%ROWTYPE; v_caller uuid:=auth.uid();
BEGIN
  IF v_caller IS NULL THEN RAISE EXCEPTION 'não autenticado'; END IF;
  IF NOT (private.has_approved_role(v_caller,'teacher') OR private.has_approved_role(v_caller,'admin')) THEN RAISE EXCEPTION 'somente professores aprovados ou administradores'; END IF;
  SELECT * INTO v_class FROM public.classes WHERE id=_class_id AND (teacher_id=v_caller OR private.has_role(v_caller,'admin')) FOR UPDATE;
  ...
  END LOOP;
  UPDATE public.classes SET join_code=v_code,join_code_expires_at=CASE WHEN _valid_days IS NULL THEN NULL ELSE now()+make_interval(days=>_valid_days) END,join_code_max_uses=_max_uses,join_code_uses=0,updated_at=now() WHERE id=_class_id;
  RETURN v_code;
END; $$;
...
BEGIN
  ...
  PERFORM pg_advisory_xact_lock(hashtextextended('lms-admin-role-change',0));
  SELECT count(*) INTO admin_count FROM public.user_roles WHERE role='admin';
  IF new_role<>'admin' AND EXISTS (SELECT 1 FROM public.user_roles WHERE user_id=target_user_id AND role='admin') AND admin_count<=1 THEN RAISE EXCEPTION 'não é possível remover o último administrador'; END IF;
...
BEGIN
  ...
  IF v_exercise_count>0 THEN
    IF EXISTS (SELECT 1 FROM public.exercises e WHERE e.lesson_id=p_lesson_id AND NOT (COALESCE(p_answers,'{}'::jsonb) ? e.id::text)) THEN RAISE EXCEPTION 'All exercises must be answered'; END IF;
    SELECT count(*) FILTER (WHERE COALESCE(p_answers,'{}'::jsonb)->>e.id::text=a.correct_answer)::integer,COALESCE(sum(e.points) FILTER (WHERE COALESCE(p_answers,'{}'::jsonb)->>e.id::text=a.correct_answer),0)::integer,COALESCE(jsonb_object_agg(e.id::text,(COALESCE(p_answers,'{}'::jsonb)->>e.id::text=a.correct_answer)),'{}'::jsonb)
    INTO v_correct_count,v_score,v_results FROM public.exercises e JOIN public.exercise_answers a ON a.exercise_id=e.id WHERE e.lesson_id=p_lesson_id;
  END IF;
  INSERT INTO public.lesson_progress(student_id,lesson_id,completed,score,completed_at) VALUES(v_user_id,p_lesson_id,true,v_score,now())
...
REVOKE EXECUTE ON FUNCTION public.create_exercise(uuid,text,jsonb,text,integer,integer) FROM PUBLIC,anon;
