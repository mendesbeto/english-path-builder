-- Move internal SECURITY DEFINER RLS helpers out of the exposed public schema.
-- Supabase recommends keeping SECURITY DEFINER helpers in a non-exposed schema
-- while explicitly qualifying them from RLS policies.
...
CREATE OR REPLACE FUNCTION private.has_approved_role(_user_id uuid, _role public.app_role)
RETURNS boolean
LANGUAGE sql
STABLE
SECURITY DEFINER
...
AS $$
  SELECT CASE
    WHEN auth.uid() IS NULL OR _user_id IS DISTINCT FROM auth.uid() THEN false
    WHEN _role = 'admin' THEN private.has_role(_user_id, 'admin')
...
CREATE OR REPLACE FUNCTION private.is_class_member(_class_id uuid, _student_id uuid)
RETURNS boolean
LANGUAGE sql
STABLE
SECURITY DEFINER
...
CREATE OR REPLACE FUNCTION private.can_access_lesson(_student_id uuid, _lesson_id uuid)
RETURNS boolean
LANGUAGE sql
STABLE
SECURITY DEFINER
  ...
  SELECT CASE
    ...
    ELSE EXISTS (
      ...
      WHERE p.id = _student_id
        AND private.has_role(_student_id, 'student')
...
REVOKE ALL ON FUNCTION private.has_approved_role(uuid, public.app_role) FROM PUBLIC, anon, authenticated;
REVOKE ALL ON FUNCTION private.is_class_member(uuid, uuid) FROM PUBLIC, anon, authenticated;
REVOKE ALL ON FUNCTION private.can_access_lesson(uuid, uuid) FROM PUBLIC, anon, authenticated;
GRANT USAGE ON SCHEMA private TO authenticated;
GRANT EXECUTE ON FUNCTION private.has_role(uuid, public.app_role) TO authenticated;
GRANT EXECUTE ON FUNCTION private.has_approved_role(uuid, public.app_role) TO authenticated;
...
ALTER POLICY "class_students_select_member_or_staff" ON public.class_students
  USING (student_id = auth.uid() OR EXISTS (
    SELECT 1 FROM public.classes c
    WHERE c.id = class_students.class_id
      AND (c.teacher_id = auth.uid() OR private.has_approved_role(auth.uid(), 'admin'))
  ));
ALTER POLICY "class_students_insert_staff" ON public.class_students
  WITH CHECK (EXISTS (
    SELECT 1 FROM public.classes c
    WHERE c.id = class_students.class_id
      AND (c.teacher_id = auth.uid() OR private.has_approved_role(auth.uid(), 'admin'))
  ));
ALTER POLICY "class_students_delete_member_or_staff" ON public.class_students
  USING (student_id = auth.uid() OR EXISTS (
    SELECT 1 FROM public.classes c
    WHERE c.id = class_students.class_id
      AND (c.teacher_id = auth.uid() OR private.has_approved_role(auth.uid(), 'admin'))
...
ALTER POLICY "classes_insert_teacher" ON public.classes
  WITH CHECK (teacher_id = auth.uid() AND (
    private.has_approved_role(auth.uid(), 'teacher')
    OR private.has_approved_role(auth.uid(), 'admin')
  ));
ALTER POLICY "classes_select_enrolled" ON public.classes
  USING (private.is_class_member(id, auth.uid()));
ALTER POLICY "classes_select_staff" ON public.classes
  USING (teacher_id = auth.uid() OR private.has_approved_role(auth.uid(), 'admin'));
...
ALTER POLICY "exercise_answers_staff_only" ON public.exercise_answers
  USING (private.has_approved_role(auth.uid(), 'teacher') OR private.has_approved_role(auth.uid(), 'admin'))
  WITH CHECK (private.has_approved_role(auth.uid(), 'teacher') OR private.has_approved_role(auth.uid(), 'admin'));

ALTER POLICY "exercises_manage_staff" ON public.exercises
  USING (private.has_approved_role(auth.uid(), 'admin') OR private.has_approved_role(auth.uid(), 'teacher'))
  WITH CHECK (private.has_approved_role(auth.uid(), 'admin') OR private.has_approved_role(auth.uid(), 'teacher'));
ALTER POLICY "exercises_select_published_or_staff" ON public.exercises
  USING (
    EXISTS (
      SELECT 1 FROM public.lessons l
      WHERE l.id = exercises.lesson_id
        AND l.is_published
        AND private.can_access_lesson(auth.uid(), l.id)
    )
    OR private.has_approved_role(auth.uid(), 'teacher')
    OR private.has_approved_role(auth.uid(), 'admin')
  );

ALTER POLICY "progress_insert_own_published_lesson" ON public.lesson_progress
  WITH CHECK (student_id = auth.uid() AND private.can_access_lesson(auth.uid(), lesson_id));
ALTER POLICY "progress_select_own_or_staff" ON public.lesson_progress
  USING (
    student_id = auth.uid()
    OR private.has_approved_role(auth.uid(), 'teacher')
    OR private.has_approved_role(auth.uid(), 'admin')
  );
ALTER POLICY "progress_update_own_published_lesson" ON public.lesson_progress
  WITH CHECK (student_id = auth.uid() AND private.can_access_lesson(auth.uid(), lesson_id));

ALTER POLICY "lessons_manage_staff" ON public.lessons
  USING (private.has_approved_role(auth.uid(), 'admin') OR private.has_approved_role(auth.uid(), 'teacher'))
  WITH CHECK (private.has_approved_role(auth.uid(), 'admin') OR private.has_approved_role(auth.uid(), 'teacher'));
ALTER POLICY "lessons_select_published_or_staff" ON public.lessons
  USING (
    (is_published AND private.can_access_lesson(auth.uid(), id))
    OR private.has_approved_role(auth.uid(), 'teacher')
    OR private.has_approved_role(auth.uid(), 'admin')
  );

ALTER POLICY "levels_manage_admin" ON public.levels
  USING (private.has_role(auth.uid(), 'admin'))
  WITH CHECK (private.has_role(auth.uid(), 'admin'));

ALTER POLICY "modules_manage_staff" ON public.modules
  USING (private.has_approved_role(auth.uid(), 'admin') OR private.has_approved_role(auth.uid(), 'teacher'))
  WITH CHECK (private.has_approved_role(auth.uid(), 'admin') OR private.has_approved_role(auth.uid(), 'teacher'));
...
ALTER POLICY "roles_manage_admin" ON public.user_roles
  USING (private.has_role(auth.uid(), 'admin'))
  WITH CHECK (private.has_role(auth.uid(), 'admin'));
ALTER POLICY "roles_select_own_or_admin" ON public.user_roles
  USING (user_id = auth.uid() OR private.has_role(auth.uid(), 'admin'));

-- Public RPCs retain only their intended business API surface. Internal helper
-- calls are now private-schema calls, so helper RPC endpoints disappear.
...
BEGIN
  ...
  IF jsonb_typeof(p_options) <> 'array' OR jsonb_array_length(p_options) < 2 THEN RAISE EXCEPTION 'At least two options are required'; END IF;
...
LANGUAGE plpgsql SECURITY DEFINER SET search_path = ''
AS $$
DECLARE v_class public.classes%ROWTYPE; v_student uuid := auth.uid();
BEGIN
  ...
  SELECT c.* INTO v_class FROM public.classes AS c
  WHERE upper(c.join_code) = upper(trim(_code)) FOR UPDATE;
  ...
  UPDATE public.classes AS c SET join_code_uses = c.join_code_uses + 1, updated_at = now() WHERE c.id = v_class.id;
  RETURN QUERY SELECT v_class.id, v_class.name;
END;
$$;

CREATE OR REPLACE FUNCTION public.regenerate_class_join_code(
  _class_id uuid, _valid_days integer DEFAULT NULL, _max_uses integer DEFAULT NULL
)
RETURNS text
LANGUAGE plpgsql SECURITY DEFINER SET search_path = ''
AS $$
...
BEGIN
  ...
  WHERE id = _class_id AND (teacher_id = auth.uid() OR private.has_role(auth.uid(), 'admin')) FOR UPDATE;
  ...
  SET join_code = v_code,
      ...
      join_code_max_uses = _max_uses, join_code_uses = 0, updated_at = now()
  WHERE id = _class_id;
  RETURN v_code;
END;
$$;
...
BEGIN
  ...
  IF target_user_id IS NULL THEN RAISE EXCEPTION 'usuário inválido'; END IF;
...
CREATE OR REPLACE FUNCTION public.submit_lesson_attempt(
  p_lesson_id uuid, p_answers jsonb DEFAULT '{}'::jsonb
)
RETURNS jsonb
LANGUAGE plpgsql SECURITY DEFINER SET search_path = ''
AS $$
DECLARE
  v_user_id uuid := auth.uid(); v_exercise_count integer := 0; v_correct_count integer := 0;
  v_score integer := 0; v_max_points integer := 0; v_results jsonb := '{}'::jsonb;
BEGIN
  ...
  SELECT count(*)::integer, COALESCE(sum(e.points), 0)::integer INTO v_exercise_count, v_max_points
  ...
  IF v_exercise_count > 0 THEN
    ...
    FROM public.exercises e JOIN public.exercise_answers a ON a.exercise_id = e.id
    WHERE e.lesson_id = p_lesson_id;
  END IF;
  ...
  RETURN jsonb_build_object('completed', true, 'score', v_score, 'max_points', v_max_points,
    'correct', v_correct_count, 'total', v_exercise_count, 'results', v_results);
END;
$$;

REVOKE EXECUTE ON FUNCTION public.has_role(uuid, public.app_role) FROM PUBLIC, anon, authenticated;
REVOKE EXECUTE ON FUNCTION public.has_approved_role(uuid, public.app_role) FROM PUBLIC, anon, authenticated;
REVOKE EXECUTE ON FUNCTION public.is_class_member(uuid, uuid) FROM PUBLIC, anon, authenticated;
REVOKE EXECUTE ON FUNCTION public.can_access_lesson(uuid, uuid) FROM PUBLIC, anon, authenticated;
